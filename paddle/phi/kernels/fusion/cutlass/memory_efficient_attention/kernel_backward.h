...
struct GmemTile {
  ...
  */
  ...
  CUTLASS_DEVICE void storeAtomicAdd(FragmentType const& fragment,
                                     int thread_id) {
    CUTLASS_PRAGMA_UNROLL
    for (int i = 0; i < kNumIters; ++i) {
...
struct AtomicLock {
  CUTLASS_DEVICE static void acquire(int32_t* lock,
                                     int set_val,
                                     int thread_id) {
    if (thread_id == 0) {
...
template <
    ...
    int kMaxK_ =
        static_cast<int>(cutlass::platform::numeric_limits<uint32_t>::max()),
...
struct AttentionBackwardKernel {
  ...
  static_assert(!kPreload || (kIsHalf && ArchTag::kMinComputeCapability >= 80 &&
                              kOutputInRF),
  ...
  static constexpr auto kOptimalAlignement = cutlass::platform::max(
      DefaultConfig::kAlignmentA, DefaultConfig::kAlignmentB);
  ...
  struct MatmulGradV {
    ...
    */
    ...
    using AccumTileGmem =
        GmemTile<typename Mma::FragmentC, static_cast<int>(kNumThreads)>;
  };
    ...
    CUTLASS_DEVICE bool advance_to_block() {
      ...
      if (kNeedsAccumGradQ || kNeedsAccumGradK || kNeedsAccumGradV) {
        ...
        workspace_gq = reinterpret_cast<GradQTempStorage*>(
            workspace_gv + workspace_elements_gv());
        if (kEnableSplitKeys) {
      ...
      PRINT_T0("[b:%d h:%d] dp[0]:%f Q:%f K:%f V:%f LSE:%f",
        static_cast<int>(blockIdx.z), static_cast<int>(blockIdx.y),
        static_cast<float>(delta_ptr[0]),
        static_cast<float>(query_ptr[0]),
        static_cast<float>(key_ptr[0]),
        static_cast<float>(value_ptr[0]),
        static_cast<float>(logsumexp_ptr[0])
      )
#endif
      return true;
    }
  ...
  struct SharedStoragePrologue {
    ...
    static void print_size() {
      // Field size
#define FSZ(f) \
  static_cast<int>((sizeof((reinterpret_cast<SharedStoragePrologue*>(0))->f)))

      printf("Total smem: %d bytes\n",
             static_cast<int>(sizeof(SharedStoragePrologue)));
...
#define FIELD_SIZEOF(f) \
  static_cast<int>((sizeof((reinterpret_cast<SharedStorageNoPrologue*>(0))->f)))
      printf("Total smem: %d bytes\n",
             static_cast<int>(sizeof(SharedStorageNoPrologue)));
...
#define FIELD(INSIDE_STRUCT, FIELDNAME) \
  ...
  using SharedStorage =
      typename cutlass::platform::conditional<kPreload,
  ...
  static bool __host__ check_supported(Params const& p) {
    ...
    if (p.num_batches > 1) {
      PADDLE_ENFORCE_EQ(p.q_strideB % kMinimumAlignment,
                        0,
                        paddle::platform::errors::InvalidArgument(
                            "query is not correctly aligned (strideB)"));
      PADDLE_ENFORCE_EQ(p.k_strideB % kMinimumAlignment,
                        0,
                        paddle::platform::errors::InvalidArgument(
                            "key is not correctly aligned (strideB)"));
      PADDLE_ENFORCE_EQ(p.v_strideB % kMinimumAlignment,
                        0,
                        paddle::platform::errors::InvalidArgument(
                            "value is not correctly aligned (strideB)"));
    }
    PADDLE_ENFORCE_EQ(p.q_strideM % kMinimumAlignment,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "query is not correctly aligned (strideM)"));
    PADDLE_ENFORCE_EQ(p.k_strideM % kMinimumAlignment,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "key is not correctly aligned (strideM)"));
    PADDLE_ENFORCE_EQ(p.v_strideM % kMinimumAlignment,
                      0,
                      paddle::platform::errors::InvalidArgument(
    ...
    if (p.grad_bias_ptr) {
      if (p.num_batches > 1) {
        PADDLE_ENFORCE_EQ(
            p.gB_strideB % kMinimumAlignment,
            0,
            paddle::platform::errors::InvalidArgument(
      ...
      if (p.num_heads > 1) {
        PADDLE_ENFORCE_EQ(
            p.gB_strideH % kMinimumAlignment,
            0,
            paddle::platform::errors::InvalidArgument(
                "attn_bias.grad is not correctly aligned (strideH)"));
      }
      PADDLE_ENFORCE_EQ(
          p.gB_strideM % kMinimumAlignment,
          0,
          paddle::platform::errors::InvalidArgument(
    ...
    PADDLE_ENFORCE_LT(p.custom_mask_type,
                      NumCustomMaskTypes,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `custom_mask_type`"));
    PADDLE_ENFORCE_LE(p.dropout_prob,
                      1.0f,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `dropout_prob`"));
    PADDLE_ENFORCE_GE(p.dropout_prob,
                      0.0f,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `dropout_prob`"));
    if (!kApplyDropout) {
    ...
    PADDLE_ENFORCE_GT(p.head_dim,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `head_dim`"));
    PADDLE_ENFORCE_GT(p.head_dim_value,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `head_dim_value`"));
    PADDLE_ENFORCE_GT(p.num_queries,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `num_queries`"));
    PADDLE_ENFORCE_GT(p.num_keys,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `num_keys`"));
    PADDLE_ENFORCE_GT(p.num_heads,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `num_heads`"));
    PADDLE_ENFORCE_GT(p.num_batches,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid value for `num_batches`"));
    PADDLE_ENFORCE_LE(p.head_dim,
                      kMaxK,
                      paddle::platform::errors::InvalidArgument(
                          "kMaxK: Expected `head_dim < kMaxK`"));
    if (kKeysQueriesAlignedToBlockSize) {
      PADDLE_ENFORCE_EQ(p.cu_seqlens_k_ptr,
                        nullptr,
                        paddle::platform::errors::InvalidArgument(
                            "This kernel does not support cu_seqlen"));
      PADDLE_ENFORCE_EQ(p.cu_seqlens_q_ptr,
                        nullptr,
                        paddle::platform::errors::InvalidArgument(
                            "This kernel does not support cu_seqlen"));
      PADDLE_ENFORCE_EQ(
          p.num_queries % kBlockSizeI,
          0,
          paddle::platform::errors::InvalidArgument(
              "kKeysQueriesAlignedToBlockSize condition not respected"));
      PADDLE_ENFORCE_EQ(
          p.num_keys % kBlockSizeJ,
          0,
          paddle::platform::errors::InvalidArgument(
              "kKeysQueriesAlignedToBlockSize condition not respected"));
    }
    if (!kEnableSplitKeys) {
      PADDLE_ENFORCE_EQ(
          p.num_splits_key,
          1,
          paddle::platform::errors::InvalidArgument("SplitKeys is disabled"));
    }
    PADDLE_ENFORCE_GT(p.num_splits_key,
                      0,
                      paddle::platform::errors::InvalidArgument(
                          "Invalid `num_splits_key` (expected >0)"));
    PADDLE_ENFORCE_LE(
        p.num_splits_key,
        cutlass::ceil_div(p.num_keys, kBlockSizeJ),
        paddle::platform::errors::InvalidArgument(
            "Invalid `num_splits_key` is too large for `num_keys`"));
    return true;
  }
  ...
  static CUTLASS_DEVICE void attention_kernel(Params p) {
    ...
    for (; key_start < p.num_keys;
      ...
      for (int32_t query_start_shifted = getQueryStart(p, key_start);
        ...
        processBlockIJ<kKeysQueriesAlignedToBlockSize>(shared_storage,
                                                       output_frags,
                                                       p,
                                                       query_start,
                                                       key_start,
  ...
  static CUTLASS_DEVICE void processBlockIJ(
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////
    {
      ...
      if (kApplyDropout) {
        ...
        PRINT_TENSOR4x4_T0_L0_START("zij", zij,
                  kBlockSizeJ - 4, kBlockSizeI - 4);
#endif

        // Save mask for later DOIVJ matmul

        int warp_idx_mn_0 = warp_id % (MatmulDOIVJ::Mma::Base::WarpCount::kM *
    ...
    for (int col = 0; col < (kSingleIterationGradQ ? 1 : p.head_dim);
      ...
      if (p.num_splits_key_device() > 1) {
        AtomicLock::acquire(&p.workspace_gq[storage_id].lock,
      ...
      if (p.num_splits_key_device() > 1) {
        ...
        isLast = __syncthreads_or(numAddsSoFar ==
    ...
    for (int col = 0; col < (kSingleIterationGradK ? 1 : p.head_dim);
      ...
      AccumTileGmem gmem_tile{p.workspace +
                              storage_id * AccumTileGmem::kElementsStored};
      if (!kOutputInRF) {
  ...
  static CUTLASS_DEVICE int32_t getSmallestQueryForKey(Params const& p,
                                                       int32_t key_start) {
    if (p.custom_mask_type == CausalFromTopLeft) {
  ...
  static CUTLASS_DEVICE void writeFragsToGmem(
    ...
    int32_t num_keys_in_block =
        skipBoundsChecks ? MatmulQK::Mma::Shape::kM
                         : cutlass::fast_min((int32_t)MatmulQK::Mma::Shape::kM,
                                             p.num_keys - key_start);
