...
static CUTLASS_DEVICE float atomicMaxFloat(float* addr, float value) {
  ...
  return (value >= 0) ? __int_as_float(atomicMax(reinterpret_cast<int*>(addr),
...
struct AttentionKernel {
  ...
  struct Params {
    ...
    CUTLASS_DEVICE bool advance_to_block() {
      ...
      if (output_accum_ptr != nullptr) {
        ...
        output_accum_ptr = reinterpret_cast<accum_t*>(output_ptr);
      }
  ...
  static bool __host__ check_supported(Params const& p) {
    ...
    if (p.causal_diagonal_ptr) {
      PADDLE_ENFORCE_NE(p.custom_mask_type,
                        NoCustomMask,
                        paddle::platform::errors::InvalidArgument(
                            "`causal_diagonal_ptr` is only useful "
                            "when `custom_mask_type` is causal"));
    }
  ...
  static void CUTLASS_DEVICE attention_kernel(Params& p) {  // NOLINT
    ...
    for (int32_t iter_key_start = 0; iter_key_start < p.num_keys;
      ...
      auto prologueV = [&](int blockN) {
        ...
        MM1::Mma::prologue(shared_storage.after_mm0.mm1,
                           iterator_V,
                           thread_id(),
                           problem_size_1_k);
      };
      ...
      for (int blockN = 0; blockN < nBlockN; ++blockN) {
        ...
        typename MM1::Mma mma_pv(
            ...
            (int)my_warp_id,   // NOLINT
            (int)my_lane_id);  // NOLINT
  ...
  CUTLASS_DEVICE static void iterative_softmax(
      ...
      cutlass::Array<accum_t, kQueriesPerBlock>& out_rescale,  // NOLINT
      cutlass::Array<accum_t,
                     kQueriesPerBlock * MM0::MmaCore::WarpCount::kN>&  // NOLINT
          addition_storage,
      int8_t lane_id,
      int8_t thread_id,
      int8_t warp_id,
